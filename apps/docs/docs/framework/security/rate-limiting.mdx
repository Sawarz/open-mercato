...
**Upload endpoints** have handler-level rate limiting keyed on `tenantId:userId` (staff) or `tenantId:sub` (customer portal):

| Endpoint | Key | Points | Window |
|----------|-----|--------|--------|
| `POST /api/attachments` | `tenantId:userId` | 30 | 60s |
| `POST /api/warranty_claims/portal/attachments` | `tenantId:sub` | 10 | 60s |

Both endpoints fail-open: if the rate-limiter service is unavailable the upload proceeds and a `warn` log is emitted.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `RATE_LIMIT_ATTACHMENTS_UPLOAD_POINTS` | `30` | Max staff attachment uploads per user per window |
| `RATE_LIMIT_ATTACHMENTS_UPLOAD_DURATION` | `60` | Window in seconds |
| `RATE_LIMIT_ATTACHMENTS_UPLOAD_BLOCK_DURATION` | — | Optional block duration after exceeding limit |
| `RATE_LIMIT_ATTACHMENTS_PORTAL_UPLOAD_POINTS` | `10` | Max portal attachment uploads per customer per window |
| `RATE_LIMIT_ATTACHMENTS_PORTAL_UPLOAD_DURATION` | `60` | Window in seconds |
| `RATE_LIMIT_ATTACHMENTS_PORTAL_UPLOAD_BLOCK_DURATION` | — | Optional block duration after exceeding limit |
